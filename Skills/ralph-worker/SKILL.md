...
4. **Commit**: Run `git add .` and `git commit -m "A descriptive description of changes"` for the changes in this step.
5. **Test**: Add unit/integration tests for the step or whole feature, make sure all tests still pass.
6. **Finalize Plan**: Mark the state file `completed: true` if all steps have been completed. Never forget this.
7. **Summarize**: When a plan is marked `completed: true`, write a concise technical summary of changes to the `learnings` array in the JSON and if applicable, provide the user with a small summary of how to manually test the feature as well.